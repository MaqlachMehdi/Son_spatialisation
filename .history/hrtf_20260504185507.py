...
class HRTF:
    ...
    ear_order: tuple[str, str] = ("left", "right")
    ...
    def get_index(self, azimuth: float, elevation: float) -> int:
        ...
        # Distance grand cercle — correcte aux pôles et pour tout azimut
        cos_d = (np.sin(el_tgt) * np.sin(el_rad)
    ...
    def from_sofa(cls, path: str | Path) -> "HRTF":
        ...
        with Dataset(str(path), "r") as f:
            hrir      = np.asarray(f.variables["Data.IR"][:])
            positions = np.asarray(f.variables["SourcePosition"][:])
            delay     = np.asarray(f.variables["Data.Delay"][:])
