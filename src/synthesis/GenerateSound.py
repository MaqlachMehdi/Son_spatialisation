...
class BurstNote:
    ...
    fade_ms         : durée du fondu en/out en millisecondes (défaut 10 ms)
                      évite les clicks aux transitions entre silence et sinus
    save            : si True, sauvegarde dans sound/generated/
    """
    ...
    def __init__(
        ...
        if offset_seconds < 0 or offset_seconds >= repeat_every:
            raise ValueError("offset_seconds doit être dans [0, repeat_every[")
    ...
    def _apply_fade(self, burst: np.ndarray) -> np.ndarray:
        fade_samples = min(int(self.sr * self.fade_ms / 1000), len(burst) // 2)
        if fade_samples == 0:
            return burst
        envelope = np.ones(len(burst), dtype=np.float32)
        # rampe cosinus : transition douce, pas de discontinuité de dérivée
        ramp = (1 - np.cos(np.linspace(0, np.pi, fade_samples))) / 2
        envelope[:fade_samples] = ramp.astype(np.float32)
        envelope[-fade_samples:] = ramp[::-1].astype(np.float32)
        return burst * envelope

    def generate(self) -> np.ndarray:
        n_total        = int(self.sr * self.duration)
        signal         = np.zeros(n_total, dtype=np.float32)
        burst          = self._apply_fade(self._note.generate())
        period_samples = int(self.sr * self.repeat_every)
        offset_samples = int(self.sr * self.offset_seconds)

        for i in range(n_total // period_samples + 1):
            start = i * period_samples + offset_samples
            end   = start + len(burst)
            if start >= n_total:
                break
            if end <= n_total:
